...
function createTokens(user) {
    const accessToken = jwt.sign({ sub: user.id, document: user.document, name: user.name, role: user.role }, env.JWT_SECRET, { expiresIn: '15m' });
    const refreshToken = jwt.sign({ sub: user.id, document: user.document, name: user.name, role: user.role }, env.JWT_REFRESH_SECRET, { expiresIn: '7d' });
