...
async function buildSummary(start, end, dateLabel) {
    ...
    const paymentBreakdown = Object.fromEntries(['efectivo', 'tarjeta', 'transferencia', 'nequi', 'daviplata'].map((m) => [
        m,
        payments.filter((p) => p.method === m).reduce((s, p) => s + p.amount, 0),
    ]));
    const ordersByStatus = Object.fromEntries(['tomado', 'en_preparacion', 'listo', 'entregado', 'pagado', 'finalizado', 'cancelado'].map((s) => [
        s,
    ...
    for (const payment of payments) {
        for (const item of payment.order.items) {
            ...
            if (!productQtys[key])
                productQtys[key] = { product: item.product, quantity: 0 };
