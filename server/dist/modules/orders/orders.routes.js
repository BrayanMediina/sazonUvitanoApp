...
router.post('/', requireAuth, requireRole('administrador', 'mesero'), validate(createSchema), async (req, res, next) => {
    try {
...
router.patch('/:id/status', requireAuth, requireRole('administrador', 'cajero'), validate(z.object({
    ...
    try {
        const order = await svc.updateStatus(req.params['id'], req.body.status);
...
router.post('/:id/items', requireAuth, requireRole('administrador', 'mesero'), validate(z.object({
    ...
    try {
        const item = await svc.addItem(req.params['id'], req.body);
...
router.delete('/:id/items/:itemId', requireAuth, requireRole('administrador', 'mesero'), async (req, res, next) => {
    try {
        const order = await svc.removeItem(req.params['id'], req.params['itemId']);
        res.json({ success: true, data: order });
    }
    catch (e) {
        next(e);
...
router.patch('/:id/cancel', requireAuth, requireRole('administrador', 'cajero'), async (req, res, next) => {
    try {
        const order = await svc.cancel(req.params['id']);
        emitToAll('order:updated', { order });
        res.json({ success: true, data: order });
    }
    catch (e) {
