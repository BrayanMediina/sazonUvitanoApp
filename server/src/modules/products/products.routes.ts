...
router.patch('/:id', ...admin, async (req, res, next) => {
  try { res.json({ success: true, data: await svc.update(req.params['id'] as string, req.body) }) } catch (e) { next(e) }
})

router.patch('/:id/toggle-availability', ...admin, async (req, res, next) => {
  try { res.json({ success: true, data: await svc.toggleAvailability(req.params['id'] as string) }) } catch (e) { next(e) }
})

router.delete('/:id', ...admin, async (req, res, next) => {
  try { await svc.remove(req.params['id'] as string); res.json({ success: true, message: 'Producto eliminado' }) } catch (e) { next(e) }
})
